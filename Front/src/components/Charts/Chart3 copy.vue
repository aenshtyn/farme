...
<template>
  <div class="lineChart">
    <div class="chartAndButtonsContainer">
      <form action="" id="cowNameSearchForm" @submit.prevent="createMilkProductionChartInSpecifiedDateHistoryRangeSETUP(15768000000000000)">
        <input name="cowNameSearch" type="text" placeholder="cow name" id="cowNameSearchBar" v-model="cowName">
        <button type="submit" class="submit btn btn-primary">Search</button>
      </form>
      <br>
      <p>Active Animal Name: {{ cowName }}</p>
      <br>
      <div class="dateRangeButtons">
        <button class = "btn btn-primary" @click="createMilkProductionChartInSpecifiedDateHistoryRangeSETUP(86400)">1 day</button>
        <button class = "btn btn-primary" @click="createMilkProductionChartInSpecifiedDateHistoryRangeSETUP(604800)">1 week</button>
        <button class = "btn btn-primary" @click="createMilkProductionChartInSpecifiedDateHistoryRangeSETUP(2630000)">1 month</button>
        <button class = "btn btn-primary" @click="createMilkProductionChartInSpecifiedDateHistoryRangeSETUP(31536000)">1 year</button>
        <button class = "btn btn-primary" @click="createMilkProductionChartInSpecifiedDateHistoryRangeSETUP(157680000)">5 years</button>
        <button class = "btn btn-primary" @click="createMilkProductionChartInSpecifiedDateHistoryRangeSETUP(10000000000000000)">All Time</button>
...
let milkProductionHistoryEpochDates = [];
let milkProductionHistoryPrices = [];
let myChart;
myChart;
export default {
  name: 'LineChart',
  mounted() {
    ...
    let createMilkProductionChartInSpecifiedDateHistoryRange = (dateRange) => {
      axios.get(FarmAPI_URL_Link)
        .then(response => {
          milkProductionHistory = response
          for (const property in milkProductionHistory.data["Monthly Adjusted Time Series"]) {
            let closingPrice = milkProductionHistory.data["Monthly Adjusted Time Series"][property]["4. close"];
        ...
        .then(() => {
          ...
          let arrayIndexWhereDateIsLargerThanDateRangeDate = milkProductionHistoryEpochDates.findIndex(isLaterDateThanDateRangeDate);
          milkProductionHistoryDates.splice(0, arrayIndexWhereDateIsLargerThanDateRangeDate);
          milkProductionHistoryPrices.splice(0, arrayIndexWhereDateIsLargerThanDateRangeDate);
          this.updateMilkProductionChart();
        });
    }
    // end of stock API code
    createMilkProductionChartInSpecifiedDateHistoryRange(23456345634563463465436);
  },
  setup() {
    ...
    let createMilkProductionChartInSpecifiedDateHistoryRangeSETUP = (dateRange) => {
